...
@adapt(QCheckBox, to=IAutomatedEditor)
class QCheckBoxIAutomatedEditorAdapter(Adapter):
    ...
    def simulate_set(self, id, delay, value):
        ...
        widget = self.adaptee
        checked = (widget.checkState() != Qt.Unchecked)

        if checked != value:
            QTest.mouseClick(widget, Qt.LeftButton, Qt.NoModifier, QPoint(),
                    delay)
